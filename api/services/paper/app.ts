...
// Processors
import * as Processors from './processor/paper';
        ...
        async (msg) => {
            try {
                ...
                switch (content.data.head) {
                    case RabbitConfig.exchangeNames.paper.queues.create:
                        await Processors.Create(processorParams);
                        break;
                    case RabbitConfig.exchangeNames.paper.queues.editPage:
                        await Processors.EditPage(processorParams);
                        break;
                    case RabbitConfig.exchangeNames.paper.queues.update:
                        await Processors.Update(processorParams);
                        break;
                    case RabbitConfig.exchangeNames.paper.queues.delete:
                        await Processors.Details(processorParams);
                        break;
                    case RabbitConfig.exchangeNames.paper.queues.list:
                        await Processors.List(processorParams);
                        break;
                    case RabbitConfig.exchangeNames.paper.queues.list:
                        await Processors.Delete(processorParams);
                        break;
                    default:
...
consoleTitle(`Getarf v${pjson.version}.${pjson.sub_version} - Paper`);